 ...
 */
    ...
    public:
		...
		float Predict(const std::unordered_map<size_t, bst_float>* feats,
        ...
        bool Transform(
            const std::unordered_map<size_t, bst_float>* feats,
            std::vector<int>& onehot_vec,
            unsigned ntree_limit, bool to_dense=false) const {
			FVec fvec;
			fvec.Set(feats);
			return TransformOneHot(fvec, onehot_vec, ntree_limit, to_dense);
		}

        inline bool TransformOneHot(FVec &feats,
                      std::vector<int>& onehot_vec,
                      unsigned ntree_limit, bool to_dense) const {
            if (ntree_limit == 0 || ntree_limit > gbm_->trees.size()) {
                ntree_limit = static_cast<unsigned>(gbm_->trees.size());
            }
            if( to_dense )
                return gbm_->getLeafOnehotDense(feats, onehot_vec, 0, ntree_limit);
            else 
                return gbm_->getLeafOnehotSparse(feats, onehot_vec, 0, ntree_limit);
        }
